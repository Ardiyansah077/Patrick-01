...
module.exports = ArdiGans = async (client = new Client(), message) => {
    try {
		...
		const creator = '6285282996146@c.us'
		...
		if (args.includes('@6285282996146')) { //replace with your bot number
	...
	case '#self':
                    ...
                    if (!isPilot) return client.reply(dari, 'Perintah ini hanya bisa di gunakan oleh Owner❗', id)
	...
	case '#unreg':
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
            let unreg = _registered.indexOf(sender.id)
                ...
                fs.writeFileSync('./ingfo/registered.json', JSON.stringify(_registered))
                return await client.reply(dari, `${pushname} telah logout dari PatrickBOT!`, id)
            break
    case '#waifu':
		if (isAfk) return await client.reply(dari, `Maaf ${pushname}, Kamu sedang afk! Ketik #unafk untuk mematikan`, id)
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
